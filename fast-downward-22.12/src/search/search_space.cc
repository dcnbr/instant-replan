...
void SearchSpace::dump(const TaskProxy &task_proxy) const {
    ofstream sslog;
    //sslog.open("relation.csv");
    sslog.open("/mnt/d/matlab/fast-downward/relation.csv");
    ...
    for (StateID id : state_registry) {
        ...
        if (node_info.creating_operator != OperatorID::no_operator &&
            ...
            sslog << node_info.parent_state_id << endl;
        } else {
            sslog << id << endl;
        }
    }
    sslog.close();
    ofstream slog;
    slog.open("statespace.csv");
    //slog.open("/mnt/d/matlab/fast-downward/statespace.csv");
    for (StateID id : state_registry) {
        State state = state_registry.lookup_state(id);
        bool first = true;
